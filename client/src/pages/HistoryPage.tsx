...
export function HistoryPage() {
  ...
  const iconBtn = (onClick: () => void, children: React.ReactNode, title: string, activeColor?: string) => (
    <button onClick={e => { e.stopPropagation(); onClick() }} title={title} style={{ padding: 6, borderRadius: 7, border: 'none', background: 'transparent', color: activeColor || '#3a3850', cursor: 'pointer', display: 'flex', alignItems: 'center', transition: 'all 0.15s' }}
      onMouseEnter={e => { e.currentTarget.style.background = 'rgba(255,255,255,0.05)'; e.currentTarget.style.color = activeColor || '#8a8699' }}
      onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; e.currentTarget.style.color = activeColor || '#3a3850' }}>
      {children}
    </button>
  )
    ...
    <div style={{ padding: '40px 48px', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', marginBottom: 32 }}>
        <div>
          ...
          <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: '2rem', fontWeight: 700, margin: 0, color: '#e8e6f0', letterSpacing: '-0.03em' }}>Historial</h1>
        </div>
        <div style={{ display: 'flex', background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 10, padding: 4, gap: 4 }}>
          {(['all', 'favorites'] as const).map(f => (
            <button key={f} onClick={() => setFilter(f)} style={{ padding: '7px 14px', borderRadius: 7, border: 'none', background: filter === f ? '#1a1a2e' : 'transparent', color: filter === f ? '#e8e6f0' : '#4a4860', fontSize: '0.8125rem', fontWeight: 500, cursor: 'pointer', transition: 'all 0.15s' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: selected ? '1fr 380px' : '1fr', gap: 20, alignItems: 'start' }}>
        ...
        <div>
          {isLoading ? (
            ...
            <div style={{ background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 16, padding: '56px 24px', textAlign: 'center' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
              {displayed.map(gen => (
                <div key={gen.id} onClick={() => setSelected(s => s?.id === gen.id ? null : gen)} style={{ background: '#0d0d14', border: `1px solid ${selected?.id === gen.id ? 'rgba(124,111,247,0.3)' : '#1a1a2e'}`, borderRadius: 14, padding: '14px 18px', cursor: 'pointer', transition: 'all 0.15s', display: 'flex', alignItems: 'center', gap: 14 }}
                  onMouseEnter={e => { if (selected?.id !== gen.id) e.currentTarget.style.borderColor = '#2a2a3a' }}
                  onMouseLeave={e => { if (selected?.id !== gen.id) e.currentTarget.style.borderColor = '#1a1a2e' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      ...
                      <span style={{ fontSize: '0.72rem', color: '#3a3850' }}>{new Date(gen.created_at).toLocaleDateString('es-AR', { day: '2-digit', month: 'short', year: 'numeric' })}</span>
                    </div>
                    <p style={{ fontSize: '0.8375rem', color: '#6b6880', margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{gen.output_text.slice(0, 100)}...</p>
                  </div>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 2, flexShrink: 0 }}>
                    {iconBtn(() => handleFavorite(gen), <svg width="15" height="15" fill={gen.is_favorite ? '#f6ad55' : 'none'} stroke={gen.is_favorite ? '#f6ad55' : 'currentColor'} strokeWidth="1.75" viewBox="0 0 24 24"><path d="M12 2l3.09 6.26L22 9.27l-5 4.87 1.18 6.88L12 17.77l-6.18 3.25L7 14.14 2 9.27l6.91-1.01L12 2z"/></svg>, 'Favorito', gen.is_favorite ? '#f6ad55' : undefined)}
                    {iconBtn(async () => { await navigator.clipboard.writeText(gen.output_text) }, <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" viewBox="0 0 24 24"><rect x="9" y="9" width="13" height="13" rx="2"/><path d="M5 15H4a2 2 0 01-2-2V4a2 2 0 012-2h9a2 2 0 012 2v1"/></svg>, 'Copiar')}
                    {iconBtn(() => handleDelete(gen), <svg width="15" height="15" fill="none" stroke="currentColor" strokeWidth="1.75" strokeLinecap="round" viewBox="0 0 24 24"><path d="M3 6h18M8 6V4h8v2M19 6l-1 14H6L5 6"/></svg>, 'Eliminar')}
                  </div>
                </div>
              ))}
              ...
              {totalPages > 1 && (
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 8 }}>
                  ...
                  <div style={{ display: 'flex', gap: 8 }}>
                    {[{ label: '← Anterior', disabled: page === 1, action: () => setPage(p => p - 1) }, { label: 'Siguiente →', disabled: page === totalPages, action: () => setPage(p => p + 1) }].map(btn => (
        ...
        {/* Detail panel */}
        {selected && (
          <div style={{ background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 16, overflow: 'hidden', position: 'sticky', top: 24 }}>
            ...
            {Object.keys(selected.input_params).length > 0 && (
              <div style={{ padding: '12px 18px', borderBottom: '1px solid #111120', background: '#080810' }}>
                <p style={{ fontSize: '0.65rem', fontWeight: 600, color: '#3a3850', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 8px' }}>Parámetros usados</p>
            ...
            <div style={{ padding: 18 }}>
              ...
              <pre style={{ margin: 0, fontFamily: "'Outfit', sans-serif", fontSize: '0.8375rem', color: '#8a8699', whiteSpace: 'pre-wrap', lineHeight: 1.7, maxHeight: 340, overflowY: 'auto' }}>{selected.output_text}</pre>
            </div>

            <div style={{ padding: '10px 18px', borderTop: '1px solid #111120', display: 'flex', justifyContent: 'space-between', background: '#080810' }}>
