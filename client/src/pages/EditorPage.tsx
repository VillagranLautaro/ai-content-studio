...
const categoryBg: Record<string, string> = { blog: 'rgba(99,179,237,0.12)', ecommerce: 'rgba(72,187,120,0.12)', social: 'rgba(159,122,234,0.12)', custom: 'rgba(246,173,85,0.12)' }
...
export function EditorPage() {
  ...
  const handleSelect = (t: Template) => {
    setSelected(t); setParams({}); reset()
    setShowParams(true) // auto-expand params on mobile after selection
  }
  ...
  const inputStyle: React.CSSProperties = { width: '100%', background: '#0a0a0f', border: '1px solid #1a1a2e', borderRadius: 10, padding: '10px 14px', color: '#e8e6f0', fontSize: '0.875rem', outline: 'none', fontFamily: "'Outfit', sans-serif", transition: 'border-color 0.2s' }
  ...
  return (
    <div style={{ padding: 'clamp(20px, 4vw, 40px) clamp(16px, 4vw, 48px)', maxWidth: 1200, margin: '0 auto' }}>
      ...
      <div style={{ marginBottom: 28 }}>
        ...
        <h1 style={{ fontFamily: "'Syne', sans-serif", fontSize: 'clamp(1.5rem, 4vw, 2rem)', fontWeight: 700, margin: 0, color: '#e8e6f0', letterSpacing: '-0.03em' }}>Editor <span style={{ color: '#7c6ff7' }}>IA</span></h1>
      </div>
      ...
      <div className="editor-grid">
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
          ...
          {/* Templates */}
          <div style={{ background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 16, padding: 14 }}>
            <p style={{ fontSize: '0.65rem', fontWeight: 600, color: '#3a3850', letterSpacing: '0.1em', textTransform: 'uppercase', margin: '0 0 10px 4px' }}>Templates</p>
            {loading ? (
          ...
          {/* Params */}
          {selected && (
            <div style={{ background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 16, overflow: 'hidden' }}>
              {/* Collapsible header on mobile */}
              <button onClick={() => setShowParams(p => !p)} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 16px', background: 'transparent', border: 'none', cursor: 'pointer', borderBottom: showParams ? '1px solid #1a1a2e' : 'none' }}>
                <p style={{ fontSize: '0.65rem', fontWeight: 600, color: '#3a3850', letterSpacing: '0.1em', textTransform: 'uppercase', margin: 0 }}>Parámetros</p>
                <svg width="14" height="14" fill="none" stroke="#3a3850" strokeWidth="2" strokeLinecap="round" viewBox="0 0 24 24" style={{ transform: showParams ? 'rotate(180deg)' : 'none', transition: 'transform 0.2s' }}>
                  <path d="M6 9l6 6 6-6"/>
                </svg>
              </button>
              {showParams && (
        ...
        {/* Output */}
        <div style={{ background: '#0d0d14', border: '1px solid #1a1a2e', borderRadius: 16, minHeight: 400, display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '14px 18px', borderBottom: '1px solid #1a1a2e' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              {isGenerating && <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5, fontSize: '0.75rem', color: '#7c6ff7', fontWeight: 500 }}><span style={{ width: 6, height: 6, borderRadius: '50%', background: '#7c6ff7', animation: 'cursor 1s infinite', display: 'inline-block' }} />Generando...</span>}
              {isComplete && <span style={{ fontSize: '0.75rem', color: '#48bb78', fontWeight: 500 }}>✓ Listo</span>}
            </div>
            {output && (
          ...
          <div style={{ flex: 1, padding: 18, display: 'flex', flexDirection: 'column', justifyContent: !output && !error ? 'center' : 'flex-start' }}>
            {!output && !isGenerating && !error && (
              <div style={{ textAlign: 'center', color: '#2e2c42' }}>
                <svg width="36" height="36" fill="none" stroke="currentColor" strokeWidth="1" strokeLinecap="round" viewBox="0 0 24 24" style={{ opacity: 0.4, marginBottom: 10 }}><path d="M12 3.5c-2 4-5 6-8.5 7 3.5 1 6.5 3 8.5 7.5 2-4.5 5-6.5 8.5-7.5-3.5-1-6.5-3-8.5-7z"/></svg>
                <p style={{ fontSize: '0.85rem', margin: 0 }}>{selected ? 'Completá los parámetros y generá' : 'Elegí un template para empezar'}</p>
              </div>
            )}
            ...
            {output && (
              <pre style={{ margin: 0, fontFamily: "'Outfit', sans-serif", fontSize: '0.875rem', color: '#c4bfdd', whiteSpace: 'pre-wrap', lineHeight: 1.75 }}>
                {output}
                {isGenerating && <span style={{ display: 'inline-block', width: 2, height: 15, background: '#7c6ff7', marginLeft: 2, verticalAlign: 'middle', animation: 'cursor 0.85s infinite' }} />}
              </pre>
            )}
          </div>
        </div>
